...
class CoefficientScaler(TransformerMixin, BaseEstimator):
    """Standardize features by removing the mean and scaling to unit variance

    The standard score of a sample `x` is calculated as:

        z = (x - u) / s * (importance **power + min_scale)
    ...
    Examples
    --------
    >>> from sklearn_expanz.preprocessing import ImportanceScaler
    >>> data = [[0, 0], [0, 0], [1, 1], [1, 1]]
    >>> y = [[0.5], [0.1], [1], [-0.5]]
    >>> scaler = ImportanceScaler()
    >>> print(scaler.fit(data, y))
    >>> print(scaler.mean_)
    ...
    def fit(self, X, sample_weight=None):
        ...
        self._reset()
        return self.partial_fit(X, sample_weight)
    ...
    def partial_fit(self, X, sample_weight=None):
        ...
        correlation_coefficients = np.corrcoef(X.T)
        total_absolute_value_cc = [np.sum(np.abs(i)) for i in correlation_coefficients]
        total_absolute_value_cc /= np.mean(total_absolute_value_cc)
        self.total_absolute_correlation_coefficients_ = total_absolute_value_cc
        ...
        if self.with_std:
            self.scale_ = (_handle_zeros_in_scale(np.sqrt(self.var_)) / 
                        self.total_absolute_correlation_coefficients_)
        else:
            self.scale_ = None
...
class ImportanceScaler(TransformerMixin, BaseEstimator):
    """Standardize features by removing the mean and scaling to unit variance
    ...
    @_deprecate_positional_args
    def __init__(self, estimator=RandomForestRegressor(random_state=0),*, copy=True, with_mean=True, with_std=True, power=0.5, min_scale=0.25):
    ...
    def partial_fit(self, X, y=None, sample_weight=None):
        ...
        if self.with_std:
            self.scale_ = (_handle_zeros_in_scale(np.sqrt(self.var_)) * 
                        (self.estimator.feature_importances_ ** self.power + self.min_scale))
        else:
            self.scale_ = None
