...
class BorutaShap(BorutaPy):
    """ Feature Selection by Boruta with shap values
    """
    def __init__(self, estimator, method='shap', n_shap_samples=200):
        """

        Parameters
        ----------
        estimator : sklearn compatible estimator
            estimator to calculate feature_importance
        method : str, optional
            feature_importance method , by default 'shap'
        n_shap_samples : int, optional
            shap.sample(x, n_shap_samples), by default 200
        """
    ...
    def _get_imp(self, X, y):
        ...
        try:
            if self.method == "shap":
                ...
                imp = explainer.shap_values(shap.sample(self.X, self.n_shap_samples))
        except Exception:
...
class SelectByBorutaShap(MetaEstimatorMixin, SelectorMixin, BaseEstimator):
    ...
    def __init__(self, estimator,
        ...
        estimator: estimator Object
            If no estimator specified then a base Random Forest will be returned otherwise the specifed estimator will
            be returned.
        min_features: int
            lower bound on the number of variables after selection.
            Should  be >= 2
        ...
        p_value: float
            ...
            would make it more strict also by making the estimator more strict could impact runtime making it slower. As it will be less likley
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.starting_X = copy.deepcopy(X)
        self.X = X
        self.y = y
        ...
        self.hits  = np.zeros(self.n_features)
        ...
        for trial in tqdm(range(self.n_trials)):
            # self.features_to_removeをself.Xから除く
            ...
            # early stopping
            if _num_features(self.X) == 0:
                break

            else:
                self.Check_if_chose_train_or_test_and_train_estimator()
    ...
    def check_estimator(self):
        ...
        """
        Checks that a estimator object has been passed as a parameter when intiializing the BorutaShap class.
        ...
        ------
        AttirbuteError
             If the estimator object does not have the required attributes.
        ...
        try:
            check_feature_importance = hasattr(self.estimator, 'feature_importances_')

        except:
            check_feature_importance = True

        if check_fit is False and check_predict_proba is False:
    ...
    def check_missing_values(self):
        ...
        estimators_to_check = ('xgb', 'catboost', 'lgbm', 'lightgbm')

        model_name = str(type(self.model)).lower()
        if X_missing or Y_missing:

            if any([x in model_name for x in estimators_to_check]):
    ...
    def Check_if_chose_train_or_test_and_train_estimator(self):

        """
        Decides to fit the estimator to either the training data or the test/unseen data a great discussion on the
        ...
        if self.train_or_test.lower() == 'test':
            ...
            self.Train_estimator(self.X_boruta_train, self.y_train, sample_weight = self.w_train)

        elif self.train_or_test.lower() == 'train':
            # estimator will be trained and evaluated on the same data
            self.Train_estimator(self.X_boruta, self.y, sample_weight = self.sample_weight)
    ...
    def Train_estimator(self, X, y, sample_weight = None):
        ...
        """
        Trains Model also checks to see if the estimator is an instance of catboost as it needs extra parameters
        also the try except is for estimators with a verbose statement
        ...
        if 'catboost' in str(type(self.estimator)).casefold():
            self.estimator.fit(X, y, sample_weight = sample_weight, cat_features = self.X_categorical,  verbose=False)

        else:

            try:
                self.estimator.fit(X, y, sample_weight = sample_weight, verbose=False)
    ...
    def calculate_hits(self):
        ...
        padded_hits = np.zeros(self.n_features)
    ...
    def create_shadow_features(self):
        ...
        self.X_shadow.columns = [f'shadow_{feature}'   for feature in self.X.columns]
    ...
    def feature_importance(self, normalize):
        ...
        """
        Caculates the feature importances scores of the estimator
        ...
        if self.importance_measure == 'shap':
            ...
            Shadow_feature_import = vals[len(self.X_shadow.columns):]
                ...
                feature_importances_ =  np.abs(self.estimator.feature_importances_)
    ...
    def explain(self):
        ...
        """
        The shap package has numerous variants of explainers which use different assumptions depending on the estimator
        ...
        ----------
            ValueError:
                if no estimator type has been specified tree as default
        """
    ...
    def test_features(self, iteration):
        ...
        modified_reject_p_values = self.bonferoni_corrections(regect_p_values,
                                                              alpha=0.05,
                                                              n_tests=len(self.columns))[1]
        
        # Take the inverse as we want true to keep featrues
        rejected_columns = np.array(modified_reject_p_values) < self.pvalue
        accepted_columns = np.array(modified_acceptance_p_values) < self.pvalue

        print("accepted_columns")
        print(accepted_columns)
        print("modified_acceptance_p_values")
        print(modified_acceptance_p_values)
        if sum(accepted_columns) < self.min_features:
            argsorted_ = np.argsort(modified_acceptance_p_values)
            print("argsorted_")
            print(argsorted_)
            threshold_ = modified_acceptance_p_values[self.min_features - 1] + 0.001
            print("threshold_")
            print(threshold_)
            accepted_columns = modified_acceptance_p_values <= threshold_
            print("accepted_columns")
            print(accepted_columns)
...
class SelectByBoruta(MetaEstimatorMixin, SelectorMixin, BaseEstimator):
    ...
    def __init__(self, estimator, 
                 ...
                 random_state=None,
                 prefit=False,
                 max_features=None, min_features=2, 
                 ):
    ...
    def _get_support_mask(self):
        ...
        if self.prefit:
            if (self.scores is not None) and (self.threshold is not None):
                ...
                raise ValueError('"prefit = True" is only applicable if support_ has already been calculated ')
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        **fit_params : Other estimator specific parameters
        ...
        feature_selector = BorutaPy(self.estimator, 
        ...
        self.support_ = feature_selector.support_
        self.scores = np.array([1 if i==True else 0 for i in  self.support_])
        self.threshold = 0.5
